...
PARAMS_JSON=$(cat <<EOF
{
  ...
  "apimSubnetPrefix": {"value": "$(echo $CONFIG_JSON | jq -r '.networking.apimSubnetPrefix // "10.0.4.0/27"')"},
  ...
  "sqlAllowedIpRules": {"value": $(echo $CONFIG_JSON | jq -c '.services.sqldb.allowedIpRules // []')},
